...
print X.shape
y=data[:,96]
#a = (np.random.rand(97,noHiddenNodes)-0.5)/5 # initial weight for alpha
a =weight(97,noHiddenNodes)
b =weight(noHiddenNodes+1,4)
#b =np.random.rand(noHiddenNodes+1,4) #initial weight for beta
...
improvement=1
#while improvement==1:
print 'yeta a =',yeta_a ,' yeta b = ',yeta_b, 'hidden nodes = ',noHiddenNodes ,'lamda = ',gamma
for i in range(0,3):
...
#    print  np.matrix(label[y[i]]),Yi
    ...
    for m in range(0,noHiddenNodes):
      for k in range(0,4):
        beta[m,k] =beta[m,k] + np.array(delta_ki)*Zi[m,:] 
    sum_sqerror_N += sum_sq_error
  #print alpha
 # print beta
  anew=a-yeta_a*(np.array(alpha) - 2*a)
#  print a
  bnew=b-yeta_b*(np.array(beta) - 2*b)
