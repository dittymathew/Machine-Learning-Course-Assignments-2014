...
def regClassify(X_train,X_test,Y_train,Y_test):
  ...
  yhat = X*beta
  print 'decision',yhat[:,0].shape,X[:,0].shape
  ...
  return (Y_predict,yhat,np.matrix(X))
...
(y_predict,yhat,Xhat) = regClassify(t_x_train,t_x_test,y_train,y_test)
...
print Xhat[:,1],yhat[:,0],t_classA[:,0]
xhat=[]
yhathat=[]
for i in range(0,t_x_train.shape[0]):
  xhat.append(t_x_train[i,0])
  yhathat.append(yhat[i,0])
#xhat =np.matrix(t_x_train[:,0]).reshape(2000)
plt.show()
...
#plt.plot(xhat,yhathat,c='green',marker ='.')
#plt.show()
fig =plt.figure()
